...
const createCheckoutSession = async (req: Request<{}, {}, CreateCheckoutInput>, res: Response) => {
    try {
        ...
        // L'ordine potrebbe non esistere: senza questo controllo, l'accesso
        // a order.products sotto avrebbe causato un crash poco chiaro (500 generico)
        if (!order) {
            return res.status(404).json({ message: "Order not found" });
        }

        // Create lines for products of each order
        const productLineItems = order.products.map((p: any) => ({
            price_data: {
                currency: 'eur',
                product_data: {
                    name: p.product.name,
