...
import ProducerProductCard, { EditData } from "../../components/ProducerProductCard";
...
function ProducerHome({ setShowGreenAssistant }: ProducerHomeProps) {
  ...
  const handleUpdateProduct = async (productId: string, editData: EditData) => {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/products/${productId}`,
      {
        method: "PUT",
