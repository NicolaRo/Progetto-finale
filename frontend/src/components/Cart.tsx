...
function Cart({ setShowCart }: CartProps) {
  ...
  const [checkoutErrors, setCheckoutErrors] = useState<CheckoutError | null>(null);

  const { user, token } = useAuth();

  // Function to calculate Cart total
  const totalPrice = cart.reduce((accumulator, product) => {
    return accumulator + Number(product.orderedQuantity) * product.price;
  }, 0);
        ...
        <div className="cart-drawer" onClick={(e) => e.stopPropagation()}>
            ...
            {cart.length === 0 ? (
              ...
              cart.map((product) => (
                <div className="cart-items" key={product.product}>
                  ...
                  >
                    <CloseIcon className="remove-from-cart-icon" size={16} aria-hidden="true" />
                  </button>
                </div>
              ))
            )}
          ...
          <div className="cart-order-btn-container">
            <button className="clear-cart-btn btn btn--ghost" onClick={() => dispatch(clearCart())}>
              <TrashIcon className="clear-cart-icon" size={16} aria-hidden="true" />
              Clear cart
            </button>
            ...
            <button
              ...
              <CheckIcon className="confirm-order-icon" size={16} aria-hidden="true" />
